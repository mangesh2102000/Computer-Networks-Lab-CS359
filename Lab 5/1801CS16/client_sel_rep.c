...
int main(int argc, char const *argv[])
{
	...
	// Declare variables to store sent and received messages
	...
	while(1){
		...
		// Receive ACK or NAK from the server, depending on the sent message order for each message
		// ACK x - represents message with sequence number x is successfully received by the server
		// NAK x - represents that message x is not received by the server
		// In the NAK scenario user may send message with sequence number x only, and then follow previous order no need to send all packages from x+1 again
		// In all ACK scenario user may send messages from sequence number x + 1 (hopefully in order)
